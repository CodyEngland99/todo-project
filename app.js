	...
	if (
		...
		childDivsWithClassName.forEach(function (childDiv) {
			...
			if (filterTasks.classList.contains("week")) {
				...
				let userDate = childDiv.value;
				let currentDate = userDate.slice(-8);

				if (isBetween(currentDate, start, end)) {
					childDiv.style.display = "flex";
				} else {
					childDiv.style.display = "none";
				}
...
function getStartOfWeek() {
	...
	// Convert to string and slice to get the day of the month
	return startOfWeek.toDateString().slice(8, 10);
}
...
function getEndOfWeek() {
	...
	// Convert to string and slice to get the day of the month
	return endOfWeek.toDateString().slice(8, 10);
}
